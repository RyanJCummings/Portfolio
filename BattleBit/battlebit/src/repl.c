        ...
        for (int j = 0; j < board_size + 1; j++) {
            ...
            if(i == 0) {
                if (j == 0) {
                    ...
                    sprintf(output + strlen(output)," %d", j - 1);
                }
            // Handle column 0
            } else if (j == 0) {
                sprintf(output + strlen(output), "%d", i - 1);
            // Handle ships
            } else {
                if(player_info->ships & xy_to_bitval(j - 1, i - 1)) {
    ...
    for (int i = 0; i < board_size + 1; i++) {
        for (int j = 0; j < board_size + 1; j++) {
            ...
            if(i == 0) {
                ...
                sprintf(output + strlen(output), "%d", i - 1);
                // Handle shots and hits
            } else {
                if(player_info->shots & xy_to_bitval(j - 1, i - 1)) {
                    if (player_info->hits & xy_to_bitval(j -1, i - 1)) {
                        sprintf(output + strlen(output), " H");
                    }
                    else {
